...
def _normalize_title(title: str) -> str:
    """
    Lowercase, strip punctuation/spaces so near-identical titles
    from different sources or categories collapse to the same key.
    e.g. "SBP cuts rate by 100bps" == "SBP Cuts Rate By 100bps!"
    """
...
def deduplicate(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove duplicate articles across all sources and categories.

    Strategy (3 passes):

    Pass 1 — Exact normalized title match
        Two articles with identical normalized titles on the same date
        are duplicates regardless of source or category. Keep the one
        from the highest-priority source (brecorder > dawn > mettis)
        since brecorder tends to have more financial detail.

    Pass 2 — Same-day near-duplicate via shared n-gram fingerprint
        Titles sharing ≥ 60% of their word 3-grams on the same date
        are considered near-duplicates (e.g. "SBP cuts rate to 11pc"
        vs "SBP reduces policy rate to 11 per cent"). Keep the longer
        title as it usually contains more information.

    Pass 3 — Cross-date exact duplicate
        Same normalized title appearing on different dates (re-published
        articles). Keep only the earliest occurrence.
    """
    before = len(df)
...
def merge_news() -> pd.DataFrame:
    ...
    # deduplicate before sentiment — saves FinBERT time
...
def run():
    ...
    # Step 1: merge + dedupe + sentiment + save + push
    df = merge_news()
    ...
    push_to_github(
        ...
        "Update news_merged.csv — filtered + deduped + FinBERT sentiment"
    )
