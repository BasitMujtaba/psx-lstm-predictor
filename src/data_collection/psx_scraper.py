...
def _push_to_github(raw_path, start, end):
    try:
        cmds = [
            ["git", "-C", PROJECT_ROOT, "pull", "--rebase", "origin", "main"],
            ["git", "-C", PROJECT_ROOT, "add", raw_path],
        ...
        log.info("Pushed raw CSV to GitHub")
...
def run(cfg=None):
    ...
    # ── Check raw cache — skip downloading if raw already exists
    raw_valid, raw_df = _raw_cache_valid(raw_path)
    if raw_valid:
        log.info("Raw cache valid — returning directly (skipping download)")
        return raw_df

    # ── Fetch from PSX
    log.info("No raw cache found — fetching from PSX ...")
    ...
    _push_to_github(raw_path, start, end)
    return raw_df
